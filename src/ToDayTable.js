...
const NEW_PAIRS = gql `
  query pairDayDatas($reserveUSD: Int!, $dailyVolumeUSD: Int!, $dailyTxns: Int!, $timeStampGt: Int!){
    pairDayDatas(where: {reserveUSD_gt: $reserveUSD, dailyVolumeUSD_gt: $dailyVolumeUSD, dailyTxns_gt: $dailyTxns, date_gt: $timeStampGt} first: 100, 
...
function ToDayTable() {
  ...
  const { loading, data } = useQuery(NEW_PAIRS, {
    variables: {
      ...
      timeStampGt : Math.floor(Date.now() / 1000) - 86400,
    }
  })
  ...
  return (
    <Grid container spacing={3} direction="row"  direction="column" justify="center" alignItems="center">
      ...
      <Typography variant="h2" color="inherit" className={classes.toolbarTitle}>Uniswap LP today so far (UTC)</Typography>
